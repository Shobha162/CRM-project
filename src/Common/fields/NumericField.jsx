...
const NumericField = ({
  ...
  return (
    ...
    >
      ...
      <div
        className="flex items-center border w-full overflow-hidden rounded-sm transition-all"
        style={{
          ...
          render={({ field }) => (
            <input
              ...
              onChange={(e) => {
                ...
                // Keep as string in input but store as number when valid
                const numVal =
                  val === "" ? "" : isNaN(Number(val)) ? val : Number(val);
                field.onChange(numVal);
              }}
              className={
                "w-full px-2.5 py-2 text-sm font-medium outline-none border-none bg-transparent disabled:cursor-not-allowed transition-all" +
                (className ? ` ${className}` : "")
              }
              style={{
